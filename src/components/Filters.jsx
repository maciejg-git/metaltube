...
import { PlaceholderSimilarBandsFilter } from "./Placeholder.jsx";
...
const Filters = ({
  ...
  return (
    <div className="flex flex-col gap-y-7">
      ...
      {(similarBands.targetBand || similarBandsLoading) && (
        <>
          ...
          {similarBandsLoading ? (
            <PlaceholderSimilarBandsFilter></PlaceholderSimilarBandsFilter>
          ) : (
            <div className="flex flex-col gap-y-6">
              <div className="text-3xl font-semibold">{similarBands.targetBand}</div>
              <div className="flex items-center justify-between">
                <div className="text-lg font-semibold">Similar bands:</div>
              </div>
              <FilterRow>
...
const SimilarBandButton = ({ name, score, active, onClick, hasAlbums }) => {
  return (
    <button
      className={clsx(
        "flex gap-x-2 rounded-full bg-gray-100 px-3 py-2 text-sm transition-colors transition-shadow hover:bg-gray-50 dark:bg-neutral-800 hover:dark:bg-neutral-700",
