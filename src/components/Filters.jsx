  ...
  return (
    <>
      <div className="flex flex-col gap-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="text-lg font-semibold">
            {activeFilters.year.size > 0
              ? `Year (${activeFilters.year.size})`
              : "Year"}
          </div>
          <div className="flex divide-x divide-gray-300 dark:divide-gray-500">
            <FilterClearButton
              onClick={() => onFilterClear("year")}
            ></FilterClearButton>
            <FilterShowAllButton
              show={yearShowAll}
        ...
        <FilterRow>
          {filters.year
            ?.slice(0, yearShowAll ? filters.year.length : 10)
            .map((name) => {
              return (
                <FilterButton
                  key={name}
...
const FilterRow = ({ children }) => {
  return (
    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 rounded-lg">
      {children}
    </div>
  );
};
